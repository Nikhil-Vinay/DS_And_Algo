...
int main()
{
   ...
   cout<<"xor sum from index 0 to index 6 : "<<query(BIT, 6)<<endl;
   cout<<"xor sum from index 2 to index 5 : "<<(query(BIT, 5)^query(BIT, 1))<<endl;
   ...
   // updateTree(arr, BIT, n, 1, arr[1]);  // it nullify at index 1 as a^a = 0;
   // updateTree(arr, BIT, n, 1, 5);
   //
   // Or we can updat with relative value of new value from previus value as below:
   // updateTree(arr, BIT, n, 1, 5^arr[1])  // a^5^a = 5

   updateTree(arr, BIT, n , 1, 5^arr[1]);  // update index 1 with 5
   cout<<"XOR Sum of range from index 0 to index 5 after update : "<< query(BIT, 5)<<endl;
